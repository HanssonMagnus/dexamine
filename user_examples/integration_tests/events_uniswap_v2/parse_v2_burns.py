...
for block in blocks:
    ...
    for index in tx_indexes:
        ...
        try:
            burns = parse_uni_v2_events.parse_v2_burns(
                logs, burn_indexes, uniswap_v2_erc20_abi, uniswap_v2_pair_abi
            )
        except Exception as e:
            logger.error(e, exc_info=True)
        ...
        for burn_index in burn_indexes:
            try:
                burn = parse_uni_v2_events.parse_v2_burn(
                    logs, burn_index, uniswap_v2_erc20_abi, uniswap_v2_pair_abi
                )
            except Exception as e:
                logger.error(e, exc_info=True)
