...
for block in blocks:
    ...
    for index in tx_indexes:
        ...
        # Parse trade data
        try:
            trades = parse_uni_v2_events.parse_v2_trades(
                logs, swap_indexes, uniswap_v2_erc20_abi, uniswap_v2_pair_abi
            )
        except Exception as e:
            logger.error(e, exc_info=True)
        ...
        for trade in trades:
            ...
            if dxt > 0:  # dxt is deposited to the liquidity pool and we take out dyt
                ...
                dyt_check_rounded = dyt_check.quantize(
                ...
                dxt_check_rounded = dxt_check.quantize(
                    Decimal("1"), rounding=ROUND_HALF_UP
                )
