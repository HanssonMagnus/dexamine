...
########################################################################################
def parse_all_v2_events(logs, erc20_abi, uniswap_v2_pair_abi, exchange_pair_address=""):
    ...
    if swap_indexes:  # If the list is not empty
        if exchange_pair_address == "":
            swaps = parse_v2_trades(logs, swap_indexes, erc20_abi, uniswap_v2_pair_abi)
            for swap in swaps:
            ...
            for swap_index in swap_indexes:
                ...
                if smart_contract == exchange_pair_address:
                    swap = parse_v2_trade(
                        logs, swap_index, erc20_abi, uniswap_v2_pair_abi
                    )
                    if swap is not None:
                        events.append(swap)
    ...
    if mint_indexes:
        if exchange_pair_address == "":
            mints = parse_v2_mints(logs, mint_indexes, erc20_abi, uniswap_v2_pair_abi)
            for mint in mints:
                if mint is not None:
                    events.append(mint)
        else:
            for mint_index in mint_indexes:
                ...
                if smart_contract == exchange_pair_address:
                    mint = parse_v2_mint(
                        logs, mint_index, erc20_abi, uniswap_v2_pair_abi
                    )
                    if mint is not None:
                        events.append(mint)
    ...
    if burn_indexes:
        if exchange_pair_address == "":
            burns = parse_v2_burns(logs, burn_indexes, erc20_abi, uniswap_v2_pair_abi)
            for burn in burns:
                if burn is not None:
                    events.append(burn)
        else:
            for burn_index in burn_indexes:
                ...
                if smart_contract == exchange_pair_address:
                    burn = parse_v2_burn(
                        logs, burn_index, erc20_abi, uniswap_v2_pair_abi
                    )
                    if burn is not None:
                        events.append(burn)
...
########################################################################################
def parse_v2_trades(logs, swap_indexes, erc20_abi, uniswap_v2_pair_abi):
    ...
    for swap_index in swap_indexes:
        try:
            trade = parse_v2_trade(logs, swap_index, erc20_abi, uniswap_v2_pair_abi)
        except Exception as e:
            logger.error(e, exc_info=True)
        trades.append(trade)
...
def parse_v2_trade(logs, swap_index, erc20_abi, uniswap_v2_pair_abi):
    """
...
########################################################################################
def parse_v2_mints(logs, mint_indexes, erc20_abi, uniswap_v2_pair_abi):
    ...
    for mint_index in mint_indexes:
        try:
            mint = parse_v2_mint(logs, mint_index, erc20_abi, uniswap_v2_pair_abi)
        except Exception as e:
            logger.error(e, exc_info=True)
        mints.append(mint)
...
def parse_v2_mint(logs, mint_index, erc20_abi, uniswap_v2_pair_abi):
    """
...
def parse_v2_burns(logs, burn_indexes, erc20_abi, uniswap_v2_pair_abi):
    ...
    for burn_index in burn_indexes:
        try:
            burn = parse_v2_burn(logs, burn_index, erc20_abi, uniswap_v2_pair_abi)
        except Exception as e:
            logger.error(e, exc_info=True)
        burns.append(burn)
...
def parse_v2_burn(logs, burn_index, erc20_abi, uniswap_v2_pair_abi):
    """
