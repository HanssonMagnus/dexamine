...
def parse_all_v3_events(logs, erc20_abi, uniswap_v3_pair_abi, exchange_pair_address=''):
    ...
    mint_indexes = general_helpers.get_event_index(topics_0, constants.uniswap_v3_mint_event)
    burn_indexes = general_helpers.get_event_index(topics_0, constants.uniswap_v3_burn_event)
        ...
        if exchange_pair_address: # If string is not empty
            for swap_index in swap_indexes:
                ...
                if smart_contract == exchange_pair_address:
                    swap = parse_v3_trade(logs, swap_index, erc20_abi, uniswap_v3_pair_abi)
...
def parse_v3_trade(logs, swap_index, erc20_abi, uniswap_v3_pair_abi):
    ...
    token_0, token_1 = uniswap_v3_parsing.get_v3_pair(swap_contract, uniswap_v3_pair_abi)
    ...
    amount0 = general_helpers.parse_signed_int(swap_data[0:64]) # pool change in token0 (- if pool sends out)
    amount1 = general_helpers.parse_signed_int(swap_data[64:128]) # pool change in token1
    sqrtPriceX96 = int(swap_data[128:192], 16) # mid-price of the pool after the swap
    liquidity = int(swap_data[192:256], 16) # liquidity of pool after the swap
    tick = general_helpers.parse_signed_int(swap_data[256:320]) # tick after the swap was executed
    ...
    price = uniswap_v3_parsing.sqrtPriceX96_to_price(sqrtPriceX96, decimals_0, decimals_1)

    # Adding 'NA' for tick_lower and tick_upper for the mint/burn events
    trade = ['swap', dex_symbol, symbol_0, symbol_1, decimals_0, decimals_1, amount0, amount1, liquidity,
        tick, sqrtPriceX96, price, 'NA', 'NA']
...
def parse_v3_mint(logs, mint_index, erc20_abi, uniswap_v3_pair_abi):
    ...
    token_0, token_1 = uniswap_v3_parsing.get_v3_pair(mint_contract, uniswap_v3_pair_abi)
    ...
    mint = ['mint', dex_symbol, symbol_0, symbol_1, decimals_0, decimals_1, amount0, amount1, liquidity,
            'NA', 'NA', 'NA', tick_lower, tick_upper]
...
def parse_v3_burn(logs, burn_index, erc20_abi, uniswap_v3_pair_abi):
    ...
    token_0, token_1 = uniswap_v3_parsing.get_v3_pair(burn_contract, uniswap_v3_pair_abi)
    ...
    amount0 = -int(burn_data[64:128], 16)  # Amount of token0 (set as negative, since out of pool)
    amount1 = -int(burn_data[128:192], 16)  # Amount of token1 (set as negative, since out of pool)

    # Transform amount0 and amount1 to base units
    amount0 = amount0*10**-decimals_0
    amount1 = amount1*10**-decimals_1

    burn = ['burn', dex_symbol, symbol_0, symbol_1, decimals_0, decimals_1, amount0, amount1, liquidity,
            'NA', 'NA', 'NA', tick_lower, tick_upper]
